...
const RecipeDetail = () => {
  ...
  const [isAdminTakedownModalOpen, setIsAdminTakedownModalOpen] =
    useState(false);
      ...
      interval = setInterval(async () => {
        try {
          ...
          if (taskData.status === "completed") {
            ...
            // 🌸 Update State dengan jawaban akhir dari AI beserta Sprite baru!
            setAiState((prev) => ({
              ...prev,
              character: {
                ...prev.character,
                ...
                // TANGKAP SPRITE BARU DARI BACKEND DI SINI:
                sprite: taskData.updated_sprite_url || prev.character.sprite,
              },
  ...
  const handleSubmitReport = async (e) => {
    ...
    if (!reportForm.reason.trim())
      return toast.error("Alasannya diisi dulu ya!");
    ...
    try {
      ...
      toast.success("Laporan berhasil dikirim! Terima kasih ya.", {
        id: toastId,
      });
  ...
  const submitAdminTakedown = async (e) => {
    e.preventDefault();
    if (!adminTakedownReason.trim())
      return toast.error("Alasannya wajib diisi, Admin!");
    ...
    try {
      ...
      await api.post(`/admin/recipes/${id}/takedown`, {
        id: id,
        reason: adminTakedownReason,
      });
  ...
  return (
    <div
      className={`flex-1 w-full flex flex-col animate-fadeIn font-sans ${isActiveSession ? "bg-gray-900 text-gray-100" : "bg-[#fbf9f7] text-gray-800"}`}
    >
      {/* HEADER GAMBAR */}
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {recipe.categories?.length > 0 ? (
              recipe.categories.map((cat, i) => (
                <span
                  key={i}
                  className="bg-[#C18A5E] text-white text-[10px] md:text-xs font-black px-3 py-1.5 rounded-full shadow-md uppercase"
                >
                  {cat}
                </span>
              ))
      ...
      <div className="px-6 md:px-12 -mt-4 relative z-10 flex-1 pb-10">
        {/* Card Info Singkat */}
        <div
          className={`${isActiveSession ? "bg-gray-800 border-gray-700" : "bg-white border-gray-50"} rounded-3xl p-5 shadow-lg border flex justify-around items-center mb-8`}
        >
          <div className="flex flex-col items-center gap-1">
            <div
              className={`w-10 h-10 rounded-full flex justify-center items-center ${isActiveSession ? "bg-orange-500/20 text-orange-400" : "bg-orange-50 text-[#C18A5E]"}`}
            >
              <FiClock size={20} />
            </div>
            ...
            <span
              className={`text-sm font-black ${isActiveSession ? "text-gray-200" : "text-gray-800"}`}
            >
              {recipe.cook_time_mins} Menit
            </span>
          </div>

          <div
            className={`w-px h-12 ${isActiveSession ? "bg-gray-700" : "bg-gray-100"}`}
          ></div>

          <div className="flex flex-col items-center gap-1">
            <div
              className={`w-10 h-10 rounded-full flex justify-center items-center ${isActiveSession ? "bg-indigo-500/20 text-indigo-400" : "bg-indigo-50 text-indigo-400"}`}
            >
              <FiUser size={20} />
            </div>
            <span className="text-[10px] text-gray-400 font-bold uppercase mt-1">
            ...
            <span
              className={`text-sm font-black line-clamp-1 ${isActiveSession ? "text-gray-200" : "text-gray-800"}`}
            >
              {recipe.author_name}
            </span>
          </div>
        ...
        {recipe.description && (
          <p
            className={`${isActiveSession ? "bg-gray-800 text-gray-300 border-gray-700" : "bg-white text-gray-600 border-gray-50"} font-medium leading-relaxed mb-8 p-5 rounded-3xl shadow-sm border`}
          >
            "{recipe.description}"
          </p>
        )}
          ...
          <div className="flex flex-col gap-3 mb-8 animate-fadeIn">
            ...
            {aiState.character && (
              <div className="bg-indigo-50 border border-indigo-100 p-5 md:p-6 rounded-3xl shadow-sm flex gap-4 md:gap-6 items-start transition-all duration-500">
                ...
                <div
                  className={`w-16 h-16 md:w-20 md:h-20 shrink-0 bg-white rounded-full border-2 border-indigo-200 shadow-md flex items-center justify-center overflow-hidden z-10 ${isPollingAI ? "animate-bounce" : ""}`}
                >
                  <ImageWithFallback
                    src={aiState.character.sprite}
                    fallbackText="Youri"
                    className="w-12 h-12 md:w-14 md:h-14 object-contain mt-2"
                  />
                </div>
                ...
                <div className="relative bg-white border border-gray-100 p-5 md:p-6 rounded-3xl rounded-tl-none shadow-sm flex-1">
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    <h4 className="font-black text-indigo-500 text-sm">
                      Youri
                    </h4>
                    {isPollingAI && (
                      <span className="flex gap-1">
                        <span className="w-1.5 h-1.5 bg-indigo-300 rounded-full animate-ping"></span>
                        <span className="w-1.5 h-1.5 bg-indigo-300 rounded-full animate-ping delay-75"></span>
                        <span className="w-1.5 h-1.5 bg-indigo-300 rounded-full animate-ping delay-150"></span>
                      </span>
                    )}
                  </div>
                  ...
                  {aiState.substitutions?.length > 0 && (
                    <div className="mt-4 pt-4 border-t border-indigo-50/50 animate-fadeIn">
                      <p className="text-[10px] md:text-xs font-black text-indigo-400 mb-2 uppercase tracking-widest">
                        Ide Substitusi Youri:
                      </p>
                      <ul className="text-xs md:text-sm font-medium text-gray-600 list-none space-y-2">
                        {aiState.substitutions.map((sub, idx) => (
                          <li
                            key={idx}
                            className="flex items-center gap-2 bg-indigo-50/30 p-2 rounded-xl"
                          >
                            <FiCheckCircle className="text-emerald-500 shrink-0" />
                            <span className="text-red-400 line-through decoration-2 font-bold">
                              {sub.missing_item.name}
                            </span>
                            <FiArrowRight className="text-gray-400 shrink-0" />
                            <span className="font-bold text-[#10b981]">
                              {sub.replaced_with.name}
                            </span>
                          </li>
                        ))}
                      </ul>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="flex flex-col gap-4">
            <h3
              className={`text-xl font-black flex items-center gap-2 ${isActiveSession ? "text-gray-100" : "text-gray-800"}`}
            >
              <div className="w-2 h-6 bg-[#C18A5E] rounded-full"></div>{" "}
              Bahan-bahan
            </h3>
            <div
              className={`${isActiveSession ? "bg-gray-800 border-gray-700" : "bg-white border-gray-100"} rounded-3xl p-6 shadow-sm border flex flex-col gap-3`}
            >
              {recipe.ingredients?.length > 0 ? (
                recipe.ingredients.map((ing, index) => (
                  <div
                    key={index}
                    className={`flex items-start gap-3 pb-3 border-b last:border-0 last:pb-0 ${isActiveSession ? "border-gray-700" : "border-gray-50"}`}
                  >
                    <FiCheckCircle
                      className="text-[#C18A5E] mt-0.5 shrink-0"
                      size={18}
                    />
                    <span
                      className={`${isActiveSession ? "text-gray-300" : "text-gray-700"} font-medium leading-snug`}
                    >
                      {ing}
                    </span>
                  </div>
          ...
          <div className="flex flex-col gap-4">
            <h3
              className={`text-xl font-black flex items-center gap-2 ${isActiveSession ? "text-gray-100" : "text-gray-800"}`}
            >
              <div className="w-2 h-6 bg-indigo-400 rounded-full"></div> Langkah
              Masak
            </h3>
            <div
              className={`${isActiveSession ? "bg-gray-800 border-gray-700" : "bg-white border-gray-100"} rounded-3xl p-6 shadow-sm border flex flex-col gap-5`}
            >
              {recipe.steps?.length > 0 ? (
                recipe.steps.map((step, index) => (
                  <div key={index} className="flex gap-4 items-start">
                    <div
                      className={`w-8 h-8 shrink-0 font-black rounded-xl flex items-center justify-center shadow-inner ${isActiveSession ? "bg-indigo-500/20 text-indigo-400" : "bg-indigo-50 text-indigo-500"}`}
                    >
                      {index + 1}
                    </div>
                    <p
                      className={`${isActiveSession ? "text-gray-300" : "text-gray-700"} font-medium leading-relaxed pt-1`}
                    >
                      {step}
                    </p>
                  </div>
      ...
      {/* ==========================================
          ...
          ========================================== */}
      <div
        className={`sticky bottom-0 w-full backdrop-blur-md border-t p-4 md:p-6 z-40 pb-safe shadow-[0_-10px_30px_rgba(0,0,0,0.15)] ${isActiveSession ? "bg-gray-900/95 border-gray-800" : "bg-white/95 border-gray-100"}`}
      >
          ...
          {!isActiveSession ? (
            ...
            <>
              <button
                ...
                className="w-full md:w-80 bg-gradient-to-r from-emerald-500 to-emerald-400 hover:from-emerald-400 hover:to-emerald-300 text-gray-900 py-4 rounded-2xl font-black text-lg shadow-[0_10px_25px_rgba(16,185,129,0.2)] transition-all active:scale-95 flex justify-center items-center gap-3"
              >
                <FiCheck strokeWidth={3} size={24} /> Selesai Memasak!
              </button>
              <button
                onClick={() => setIsCancelModalOpen(true)}
                className="w-full md:w-auto bg-gray-800 text-red-400 hover:bg-red-500/20 hover:text-red-300 border border-gray-700 py-4 px-8 rounded-2xl font-bold transition-all"
              >
                Batalkan Sesi
              </button>
            </>
      ...
      {isStartModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white w-full max-w-sm rounded-[32px] p-8 shadow-2xl relative flex flex-col">
            <h2 className="text-2xl font-black text-gray-800 mb-3 text-center">
              Persiapan Memasak
            </h2>
            <p className="text-sm text-gray-500 font-medium text-center mb-8 leading-relaxed">
              Punya bahan yang kurang? Biar AI Youri bantu carikan{" "}
              <span className="font-bold text-[#C18A5E]">bahan pengganti</span>{" "}
              dari sisa dapurmu!
            </p>
            <div className="flex flex-col gap-3">
              <button
                onClick={() => {
                  setIsStartModalOpen(false);
                  navigate(`/cooking/recipe/${id}/compare`);
                }}
                className="w-full py-4 rounded-2xl font-black text-white bg-[#C18A5E] hover:bg-[#a6744d] shadow-[0_8px_20px_rgba(193,138,94,0.3)] transition-all active:scale-95"
              >
                Cari Bahan Pengganti
              </button>
              <button
                onClick={handleStartCooking}
                className="w-full py-4 rounded-2xl font-black text-gray-500 bg-gray-100 hover:bg-gray-200 transition-all"
              >
                Lewati & Langsung Masak
              </button>
              <button
                onClick={() => setIsStartModalOpen(false)}
                className="w-full py-2 mt-2 text-sm font-bold text-gray-400 hover:text-gray-600"
              >
                Tutup
              </button>
            </div>
      ...
      {isFinishModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/80 backdrop-blur-md animate-fadeIn">
          <div className="bg-gray-800 w-full max-w-sm rounded-[32px] p-8 shadow-2xl border border-gray-700 relative flex flex-col">
            ...
            <h2 className="text-2xl font-black text-white mb-2 text-center mt-2">
              Masakan Selesai!
            </h2>
            <p className="text-sm text-gray-400 font-medium text-center mb-6 leading-relaxed">
              Satu langkah lagi! Ambil foto mahakaryamu untuk mengklaim{" "}
              <span className="font-bold text-yellow-400">Bonus XP</span> dari
            ...
            <label className="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed border-gray-600 hover:border-[#C18A5E] bg-gray-900 rounded-3xl cursor-pointer transition-colors overflow-hidden relative mb-6">
              {proofImage ? (
                <>
                  ...
                  />
                  <div className="absolute flex items-center gap-2 text-white font-bold bg-black/60 px-4 py-2 rounded-full backdrop-blur-sm">
                    <FiCheckCircle className="text-emerald-400" /> Foto Siap!
                  </div>
                </>
              ) : (
                <div className="flex flex-col items-center text-gray-400 px-4 text-center">
                  <div className="w-14 h-14 bg-gray-800 rounded-full flex items-center justify-center mb-3 text-gray-300 shadow-inner">
                    <FiCamera size={24} />
                  </div>
            ...
            <button
              ...
              className="w-full py-4 rounded-2xl font-black text-gray-900 bg-gradient-to-r from-emerald-500 to-emerald-400 hover:from-emerald-400 hover:to-emerald-300 shadow-[0_8px_20px_rgba(16,185,129,0.3)] transition-all active:scale-95 disabled:opacity-50"
            >
      ...
      {isCancelModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/80 backdrop-blur-md animate-fadeIn">
          <div className="bg-gray-800 w-full max-w-sm rounded-[32px] p-8 shadow-2xl border border-gray-700 relative flex flex-col">
            <div className="w-16 h-16 bg-red-500/20 text-red-500 rounded-full flex items-center justify-center mx-auto mb-4 border border-red-500/30 shadow-inner">
              <FiX size={32} />
            </div>
            ...
            <p className="text-sm text-gray-400 font-medium text-center mb-8 leading-relaxed">
              Yakin ingin membatalkan masakan ini? Kamu{" "}
              <span className="font-bold text-red-400">tidak akan</span>{" "}
              mendapatkan reward apapun.
            </p>
            <div className="flex flex-col gap-3">
              <button
                onClick={handleCancelCooking}
                className="w-full py-4 rounded-2xl font-black text-white bg-red-500 hover:bg-red-600 shadow-[0_8px_20px_rgba(239,68,68,0.3)] transition-all active:scale-95"
              >
                Ya, Batalkan Sesi
              </button>
              <button
                onClick={() => setIsCancelModalOpen(false)}
                className="w-full py-4 rounded-2xl font-black text-gray-300 bg-gray-700 hover:bg-gray-600 border border-gray-600 transition-all"
              >
                Tidak, Lanjut Masak!
              </button>
            </div>
            ...
            <div className="w-full bg-gray-50 border border-gray-100 rounded-3xl p-5 mb-8 shadow-sm">
              ...
              <div className="w-full h-4 bg-gray-200 rounded-full overflow-hidden shadow-inner relative">
                <div
                  ...
                  style={{
                    width: `${(finishData.gamification?.current_xp / finishData.gamification?.next_xp) * 100}%`,
                  }}
                ></div>
              </div>
              <div className="text-right mt-1.5">
                <span className="text-[10px] font-bold text-gray-400">
                  {finishData.gamification?.current_xp} /{" "}
                  {finishData.gamification?.next_xp} XP
                </span>
              </div>
            </div>
            <button
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white w-full max-w-sm rounded-[32px] p-8 shadow-2xl relative">
            ...
            <p className="text-xs text-gray-500 font-medium mb-6">
              Bantu Youri menjaga kualitas dapur kita. Apa yang salah dengan
              resep ini?
            </p>
            ...
            <form onSubmit={handleSubmitReport} className="flex flex-col gap-4">
              <div className="flex flex-col gap-2">
                ...
                >
                  <option value="fake_recipe">
                    Resep Palsu / Tidak Masuk Akal
                  </option>
                  <option value="spam">Spam / Promosi Iklan</option>
                  <option value="inappropriate">
                    Gambar/Teks Tidak Pantas
                  </option>
                  <option value="irrelevant">
                    Tidak Sesuai Kategori/Judul
                  </option>
                </select>
              </div>
              ...
              <div className="flex gap-3 mt-4">
                ...
                <button
                  ...
                  className="flex-1 py-3.5 bg-red-500 hover:bg-red-600 text-white rounded-xl font-bold text-sm shadow-md transition-all disabled:opacity-50"
                >
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-gray-900/80 backdrop-blur-md animate-fadeIn">
          <div className="bg-white w-full max-w-sm rounded-[32px] p-8 shadow-2xl relative border-2 border-red-500/20 overflow-hidden">
            ...
            <p className="text-xs text-gray-500 font-medium mb-6 leading-relaxed">
              Tindakan ini akan{" "}
              <strong className="text-red-500">menghapus paksa</strong> resep
              dari peredaran dan mengirimkan notifikasi pelanggaran ke pemilik
              resep.
            </p>

            <form
              onSubmit={submitAdminTakedown}
              className="flex flex-col gap-4"
            >
              <div className="flex flex-col gap-2">
                <label className="text-xs font-bold text-gray-700">
                  Alasan Takedown (Wajib)
                </label>
                <textarea
                  rows="3"
                  required
              ...
              <div className="flex gap-3 mt-4">
                <button
                  type="button"
                  onClick={() => setIsAdminTakedownModalOpen(false)}
                  className="flex-1 py-3.5 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-xl font-bold text-sm transition-colors"
                >
                  Batal
                </button>
                <button
                  ...
                  className="flex-[1.5] py-3.5 bg-red-600 hover:bg-red-700 text-white rounded-xl font-black text-sm shadow-[0_8px_20px_rgba(220,38,38,0.3)] transition-all active:scale-95 disabled:opacity-50"
                >
