...
import { 
  ...
  FiClock, FiSearch, FiChevronLeft, FiChevronRight 
} from "react-icons/fi";
...
const MyRecipes = () => {
  ...
  useEffect(() => {
    const fetchMyRecipes = async () => {
      setIsLoading(true);
      try {
        const response = await api.get(`/user/recipes?search=${activeSearch}&page=${page}&limit=12`); 
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setRecipes(prev => prev.filter(r => r.recipe_id !== deleteModal.recipeId));
      
    } catch (error) {
      console.error("Delete Recipe Error:", error);
      toast.error(error.response?.data?.message || "Gagal menghapus resep.", { id: toastId });
    }
  };

  return (
    <div className="flex-1 w-full bg-transparent text-gray-800 font-sans pb-32 flex flex-col relative h-full">
      ...
      <div className="sticky top-0 z-30 bg-white/90 backdrop-blur-md px-6 md:px-10 py-5 flex items-center justify-between border-b border-gray-100 shadow-sm shrink-0">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-black text-gray-800 tracking-tight">Koleksi Resepmu</h1>
        </div>
      </div>
      ...
      <div className="px-6 md:px-10 py-6 flex-1 overflow-y-auto">
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center bg-gray-50 rounded-3xl border border-gray-100 border-dashed">
            ...
            <h3 className="text-lg font-black text-gray-800 mb-2">Resep Tidak Ditemukan</h3>
            <p className="text-sm text-gray-500 font-medium">
              {activeSearch ? "Coba gunakan kata kunci pencarian yang lain." : "Kamu belum membuat resep satupun. Yuk mulai berkreasi!"}
            </p>
          </div>
        ) : (
          // COMPACT GRID LAYOUT (Jauh lebih rapi dan hemat tempat!)
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {recipes.map((recipe) => (
              <div 
                key={recipe.recipe_id} 
                className="bg-white rounded-2xl p-3 shadow-sm border border-gray-100 flex items-center gap-4 hover:shadow-md hover:border-[#E3CBB8] transition-all group"
              >
                {/* Thumbnail Kecil */}
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="font-black text-base text-gray-800 truncate mb-1" title={recipe.title}>
                    {recipe.title}
                  </h3>
                  <p className="text-[10px] text-gray-400 font-bold tracking-wide mb-2">
                    {recipe.created_at}
                  </p>
                  
                  <span className={`text-[9px] font-black uppercase px-2 py-1 rounded border inline-block ${
                    recipe.status === 'published' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-red-50 text-red-500 border-red-100'
                  }`}>
                ...
                <div className="flex flex-col gap-2 shrink-0 border-l border-gray-50 pl-3">
                  <button onClick={() => navigate(`/my-recipes/edit/${recipe.recipe_id}`)} className="w-8 h-8 rounded-full bg-gray-50 hover:bg-[#E3CBB8]/20 flex justify-center items-center text-gray-500 hover:text-[#C18A5E] transition-colors" title="Edit Resep">
                    <FiEdit2 size={14} />
                  </button>
                  <button onClick={() => handleDeleteClick(recipe.recipe_id)} className="w-8 h-8 rounded-full bg-gray-50 hover:bg-red-50 flex justify-center items-center text-gray-500 hover:text-red-500 transition-colors" title="Hapus Resep">
                    <FiTrash2 size={14} />
                  </button>
                </div>
              </div>
      ...
      {deleteModal.isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white w-full max-w-sm rounded-[32px] p-8 shadow-2xl relative flex flex-col items-center text-center">
            ...
            <h2 className="text-2xl font-black text-gray-800 mb-2">Hapus Resep?</h2>
            <p className="text-sm text-gray-500 font-medium leading-relaxed mb-8">
              Resep ini akan dihapus secara permanen dari buku resepmu. Kamu yakin ingin membuangnya?
            </p>
